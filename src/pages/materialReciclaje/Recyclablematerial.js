...
class Recyclablematerial extends Component {
    state = {
        selectedRowKeys: [],
        data: [],
        columns: [],
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                return <Button type="primary" size="small" shape="circle" icon={<EditFilled
                    onClick={() =>
                        this.editRecyclablematerial(record)
                    }
                />} />
            },
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                return <Button type="primary" size="small" shape="circle" icon={<DeleteFilled
                    onClick={() =>
                        this.eliminarRecyclablematerial(record)
                    }
                />} />
            },
            //render: (text, record) => <Link to={'user/' + record.name}>{text}</Link>
    ...
    getInformation = async () => {
        await axios.get('http://localhost:3001/api/Recyclablematerial')
            .then(response => {
    ...
    editInformation = async (active, description, id, name) => {
        const param = {
            "active": active,
    ...
    insertInformation = async (active, description, id, name) => {
        const param = {
            "active": active,
            "description": description,
            "id": id,
    ...
    guardarRegistro = async () => {//nuevo y editar
        ...
        if (this.state.editNew == 1) {
            await this.insertInformation(this.state.fields["active"], this.state.fields["description"], this.state.fields["id"], this.state.fields["name"]);
            ...
            await this.editInformation(this.state.fields["active"], this.state.fields["description"], this.state.fields["id"], this.state.fields["name"]);
    ...
    modalForm = () => {
        return (
            < div >
                ...
                < Modal title="recyclablematerial"
