...
class Templateemail extends Component {
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                return <Button type="primary" size="small" shape="circle" icon={<DeleteFilled
                    onClick={() =>
                        this.eliminarTemplateemail(record)
                    }
                />} />
            },
            //render: (text, record) => <Link to={'user/' + record.name}>{text}</Link>
    ...
    handleValidationDetail() {
        ...
        if (!fields["template"]) {
            formIsValid = false;
            errors["template"] = "Campo obligatorio";
        }
    ...
    getInformation = async () => {
        await axios.get('http://localhost:3001/api/Templateemail')
            .then(response => {
    ...
    editInformation = async (active, description, id, name, template) => {
        const param = {
            "active": active,
    ...
    insertInformation = async (active, description, id, name, template) => {
        const param = {
            "active": active,
            "description": description,
            "id": id,
    ...
    guardarRegistro = async () => {//nuevo y editar
        ...
        if (this.state.editNew == 1) {
            await this.insertInformation(this.state.fields["active"], this.state.fields["description"], this.state.fields["id"], this.state.fields["name"], this.state.fields["template"]);
            ...
            await this.editInformation(this.state.fields["active"], this.state.fields["description"], this.state.fields["id"], this.state.fields["name"], this.state.fields["template"]);
